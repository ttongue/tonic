...
def buildMemberFormLine(plan,num_months=1):
   plainTxt=getPlainTextMemberPlanDescription(plan)
   setup = getSetupFeeFromPlan(plan)
   rate = getMonthlyRateFromPlan(plan)
   line="<input type=hidden name=memberplan value='"+plan+"'>"+plainTxt+" - %s month(s) included, then $" % num_months
...
def getPlanFromForm():
   ...
   if ((plan == "comped") | (plan =="associate") | (plan =="full") | (plan == "full_student") | (plan == "super") | (plan == "super_student") | (plan =="full_family") | (plan == "full_student_family") | (plan == "super_family") | (plan == "super_student_family") | (plan == "spectrvm_standard") | (plan == "spectrvm_onsite") | (plan == "spectrvm_colo")):
	return plan
...
def stepOneSelectPlan():
   outText=memberSystem.loadTemplate("templates/certSignup-step1.html");
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","Prepaid Signup - Step 1")
   outText=outText.replace("<!--ERROR_MESSAGE-->",errorText)
   outText=tmpheader+outText+footer
   print outText;

def stepTwoGatherInfo(plan,num_months):
   template=memberSystem.loadTemplate("templates/certSignup-step2.html")
   membershipLine=buildMemberFormLine(plan,num_months)
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","Prepaid Signup - Step 2")
   outText=template
   ...
   outText=outText.replace('#COUPONCODE#',cgi.escape(form.getfirst("couponcode","")))
   outText=outText.replace("<!--ERROR_MESSAGE-->",errorText)
   # invoiceHTML=getInvoiceHTML(plan)
   # outText=outText.replace("#CURRENT_INVOICE#",invoiceHTML)
   print tmpheader+outText+footer

def stepThreeConfirmation(plan,importantMessage,num_months=1):
   template=memberSystem.loadTemplate("templates/certSignup-step3.html")
   ...
   template=template.replace('#COUPONCODE#',cgi.escape(form.getfirst("couponcode","")))
   if (inspectionResults !=""):
      tmpheader=tmpheader.replace("#PAGE_TITLE#","Prepaid Signup - Step 2 - TRY AGAIN - COMPLETE ALL FIELDS")
      template=memberSystem.loadTemplate("templates/certSignup-step2.html")
      template=template.replace("#MEMBERSHIP_LEVEL#",membershipLine)
	...
	outText=outText.replace("<!--ERROR_MESSAGE-->",importantMessage)
      ...
      tmpheader=tmpheader.replace("#PAGE_TITLE#","Prepaid Signup - Step 3 - Confirmation")
      outText=outText.replace("#FORM_PARSE_RESULTS#",inspectionResults)
   #invoiceHTML=" "
   #invoiceHTML=getInvoiceHTML(plan)
   #outText=outText.replace("#CURRENT_INVOICE#",invoiceHTML)
...
def stepFourProcess(plan):
   template=memberSystem.loadTemplate("templates/certSignup-step4.html")
   results="<ul> "
   successmessage=" "
   inspectionResults=inspectFormInputsForMemberInfo()
#    inspectionResults=inspectionResults+inspectFormInputsForCreditCardInfo()
   if (inspectionResults!=""):
      ...
      couponcode=cgi.escape(form.getfirst("couponcode",""))
      num_months = memberSystem.getPassNumMonths(couponcode)
...
# 
      ...
      if (results.find("Error:") == -1):
           endDate=getEndDateYYYY_MM_DD(num_months)
           results="<li>Membership created for %s %s with member number %s expiring on %s" % (first_name,last_name,member_id,endDate)
           ...
           memberSystem.setMembershipExpirationDate(member_id,endDate)
   ...
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","Prepaid Signup - Results")
   print tmpheader
   print outText
   print footer
   additionalComments=""
   adminEmail=memberSystem.loadTemplate("templates/certStepFourEmail.txt")
   ...
   mailsystem.sendEmail('treasurer@techvalleycenterofgravity.com','ttongue@techvalleycenterofgravity.com','Prepaid Signup System Notice: %s %s'%(first_name,last_name),adminEmail)
   if (results.find("Error:") == -1):
       userEmail=memberSystem.loadTemplate("templates/certSignupEmail.txt")
       ...
       mailsystem.sendEmail('treasurer@techvalleycenterofgravity.com',email,'Membership Signup complete for %s %s'% (first_name,last_name),userEmail)
...
def processFormDataIntoTemplate(template,stump,tail):
   ...
   outTempl=outTempl.replace('#COUPONCODE#',stump+cgi.escape(form.getfirst("couponcode",""))+tail)
...
def getEndDate(numMonths=1):
   ...
   if (numMonths > 1):
     thisMonth=thisMonth+numMonths-1
   if (thisMonth>12):
...
def getEndDateYYYY_MM_DD(numMonths=1):
   today=datetime.date.today()
   thisDay=int(today.strftime('%d'))
   thisMonth=int(today.strftime('%m'))
   thisYear=int(today.strftime('%Y')) 
   if (thisDay > 10):
     thisMonth=thisMonth+1
   if (num_months > 1):
     thisMonth=thisMonth+num_months-1
   if (thisMonth>12):
     thisMonth=thisMonth-12
     thisYear=thisYear+1
   (junk,daysInMonth)=monthrange(thisYear,thisMonth)
   endOfMonth="%d-%d-%d"%(thisYear,thisMonth,daysInMonth)
   return endOfMonth
...
try:
   ...
   if (step == ""):
      ...
      couponcode=cgi.escape(form.getfirst("couponcode",""))
      if ((memberplan=="") | (couponcode=="")):
        errorText="Coupon Code REQUIRED!"
	stepOneSelectPlan()
        print "<!--"
        sys.exit(0)
      num_months = memberSystem.getPassNumMonths(couponcode)
      sponsorRecord=memberSystem.getMemberRecordFromDayPass(couponcode) 
      if (sponsorRecord[memberSystem.DBCOL_MEMBER_NUMBER] != 1):
        # It's not authorized for generating new members, or has been used
        errorText="Invalid OR Already Used Coupon Code (404)!"
        stepOneSelectPlan()
        print "<!--"
        sys.exit(0) 
      if (memberSystem.isDayPassUnused(couponcode) == 0):
	# It's not authorized for generating new members, or has been used
        errorText="Invalid OR Already Used Coupon Code! (301)"
        stepOneSelectPlan()
        print "<!--"
        sys.exit(0)
      if (memberSystem.getPassNumMonths(couponcode) < 1):
	# It's not authorized for generating new members, or has been used
        errorText="Invalid OR Already Used Coupon Code! (403)"
        stepOneSelectPlan()
        print "<!--"
        sys.exit(0)
      step=cgi.escape(form.getfirst("next_step","2"))
      if (step == "2"):
            stepTwoGatherInfo(memberplan,num_months)
      else:
	    if (step == "3"):
		stepThreeConfirmation(memberplan,"",num_months)
	    else:
		stepFourProcess(memberplan)
                memberSystem.markDayPassAsUsed(couponcode)
