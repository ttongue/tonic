...
def getPlanFromForm():
   ...
   if ((plan =="associate") | (plan =="full") | (plan == "full_student") | (plan == "super") | (plan == "super_student") | (plan =="full_family") | (plan == "full_student_family") | (plan == "super_family") | (plan == "super_student_family")):
	return plan
...
def stepOneSelectPlan():
   outText=memberSystem.loadTemplate("templates/memberSignup-step1-selectPlan.html");
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Membership Signup - Step 1")
   outText=tmpheader+outText+footer
   print outText;

def stepTwoGatherInfo(plan):
   template=memberSystem.loadTemplate("templates/memberSignup-step2-GetMemberInfo.html")
   membershipLine=buildMemberFormLine(plan)
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Membership Signup - Step 2")
   outText=template
...
def stepThreeConfirmation(plan,importantMessage):
   template=memberSystem.loadTemplate("templates/memberSignup-step3Confirmation.html")
   ...
   if (inspectionResults !=""):
      tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Membership Signup - Step 2 - TRY AGAIN - COMPLETE ALL FIELDS")
      template=memberSystem.loadTemplate("templates/memberSignup-step2-GetMemberInfo.html")
      template=template.replace("#MEMBERSHIP_LEVEL#",membershipLine)
	...
	outText=outText.replace("<!--ERROR_MESSAGE-->",importantMessage)
      ...
      tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Membership Signup - Step 3 - Confirmation")
      outText=outText.replace("#FORM_PARSE_RESULTS#",inspectionResults)
   invoiceHTML=" "
   invoiceHTML=getInvoiceHTML(plan)
   outText=outText.replace("#CURRENT_INVOICE#",invoiceHTML)
...
def inspectFormInputsForMemberInfo():
   ...
   if not re.match(r"^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-z]{2,4})$", cgi.escape(form.getfirst("email",""))):
   ...
   if not re.match(r"^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-z]{2,4})$", cgi.escape(form.getfirst("emergency_email",""))):
...
def stepFourProcess(plan):
   template=memberSystem.loadTemplate("templates/memberSignup-step4.html")
   results="<ul> "
   successmessage=" "
   inspectionResults=inspectFormInputsForMemberInfo()
   ...
   mailsystem.sendEmail('treasurer@techvalleycenterofgravity.com',tvcog.TVCOG_SIGNUP_ADMIN_NOTIFICATION_LIST,'New Member Signup System Notice: %s %s'%(first_name,last_name),adminEmail)
   if (results.find("Error:") == -1):
       userEmail=memberSystem.loadTemplate("templates/memberSignupEmail.txt")
