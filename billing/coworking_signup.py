...
def getPlanFromForm():
   ...
   if ((plan =="associate") | (plan =="full") | (plan == "full_student") | (plan == "super") | (plan == "super_student") | (plan =="full_family") | (plan == "full_student_family") | (plan == "super_family") | (plan == "super_student_family") | (plan == "coworking")):
	return plan
...
def stepOneSelectPlan():
   outText=memberSystem.loadTemplate("templates/coworkingSignup-step1.html");
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Coworking Membership Signup - Step 1")
   outText=tmpheader+outText+footer
   print outText;

def stepTwoGatherInfo(plan):
   template=memberSystem.loadTemplate("templates/coworkingSignup-step2.html")
...
def stepThreeConfirmation(plan,importantMessage):
   template=memberSystem.loadTemplate("templates/coworkingSignup-step3.html")
...
def stepFourProcess(plan):
   template=memberSystem.loadTemplate("templates/coworkingSignup-step4.html")
   results="<ul> "
   successmessage=" "
   inspectionResults=inspectFormInputsForMemberInfo()
   ...
   if (results.find("Error:") == -1):
       userEmail=memberSystem.loadTemplate("templates/coworkingSignupEmail.txt")
