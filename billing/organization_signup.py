...
def getMonthlyRateFromPlan(plan):
   ...
   if (plan == "super_student_family"):
	return 90
   if (plan == "organization_1"):
	return 200
   if (plan == "organization_2"):
	return 400
   if (plan == "organization_3"):
	return 600
   if (plan == "organization_4"):
	return 800
   if (plan == "organization_5"):
	return 1000
   return 1000
...
def getPlainTextMemberPlanDescription(plan):
   ...
   if (plan == "coworking"):
	return "Coworking"
   if (plan == "organization_1"):
	return "Organization Membership - 1 Seat"
   if (plan == "organization_2"):
	return "Organization Membership - 2 Seat"
   if (plan == "organization_3"):
	return "Organization Membership - 3 Seat"
   if (plan == "organization_4"):
	return "Organization Membership - 4 Seat"
   if (plan == "organization_5"):
	return "Organization Membership - 5 Seat"
   return plan
  

def getPlanFromForm():
   plan = cgi.escape(form.getfirst("memberplan",""))
   if ((plan =="associate") | (plan =="full") | (plan == "full_student") | (plan == "super") | (plan == "super_student") | (plan =="full_family") | (plan == "full_student_family") | (plan == "super_family") | (plan == "super_student_family") | (plan == "coworking") | (plan =="organization_1") | (plan =="organization_2") | (plan =="organization_3") | (plan =="organization_4") | (plan =="organization_5")):
	return plan
   ...
   if (planRoot == "associate"):
	return "associate"
   if (planRoot == "coworking"):
	return "coworking"
   if (planRoot == "organization_1"):
	return "organization_1"
   if (planRoot == "organization_2"):
	return "organization_2"
   if (planRoot == "organization_3"):
	return "organization_3"
   if (planRoot == "organization_4"):
	return "organization_4"
   if (planRoot == "organization_5"):
	return "organization_5"
...
def stepOneSelectPlan():
   outText=memberSystem.loadTemplate("templates/organizationSignup-step1.html");
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Organization Membership Signup - Step 1")
   outText=tmpheader+outText+footer
   print outText;

def stepTwoGatherInfo(plan):
   template=memberSystem.loadTemplate("templates/organizationSignup-step2.html")
   membershipLine=buildMemberFormLine(plan)
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Organization Membership Signup - Step 2")
   outText=template
...
def stepThreeConfirmation(plan,importantMessage):
   template=memberSystem.loadTemplate("templates/organizationSignup-step3.html")
   ...
   if (inspectionResults !=""):
      tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Organization Membership Signup - Step 2 - TRY AGAIN - COMPLETE ALL FIELDS")
      template=memberSystem.loadTemplate("templates/organizationSignup-step2.html")
      template=template.replace("#MEMBERSHIP_LEVEL#",membershipLine)
	...
	outText=outText.replace("<!--ERROR_MESSAGE-->",importantMessage)
      ...
      tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Organization Membership Signup - Step 3 - Confirmation")
      outText=outText.replace("#FORM_PARSE_RESULTS#",inspectionResults)
   invoiceHTML=" "
   invoiceHTML=getInvoiceHTML(plan)
   outText=outText.replace("#CURRENT_INVOICE#",invoiceHTML)
...
def stepFourProcess(plan):
   template=memberSystem.loadTemplate("templates/organizationSignup-step4.html")
   results="<ul> "
   successmessage=" "
   inspectionResults=inspectFormInputsForMemberInfo()
...
#	over18=1
	...
	thisDay=int(today.strftime('%d'))
      ...
      if (results.find("Error:") == -1):
           memberSystem.createSeltzerContactRecords(first_name,last_name,address,city,state,zip,email,phone,over18,emergency_contact,emergency_relationship,emergency_phone,emergency_email,plan,organization)
   ...
   tmpheader=header
   tmpheader=tmpheader.replace("#PAGE_TITLE#","TVCOG Member Signup - Results")
   print tmpheader
   print outText
   print footer
   additionalComments=""
   adminEmail=memberSystem.loadTemplate("templates/organizationStepFourEmail.txt")
   ...
   mailsystem.sendEmail('treasurer@techvalleycenterofgravity.com','ttongue@techvalleycenterofgravity.com,ttongue@thomastongue.com','New Organization Member Signup System Notice: %s %s'%(first_name,last_name),adminEmail)
   if (results.find("Error:") == -1):
       userEmail=memberSystem.loadTemplate("templates/organizationSignupEmail.txt")
       ...
       mailsystem.sendEmail('treasurer@techvalleycenterofgravity.com',email,'TVCOG Membership Notice - Membership Signup complete for %s %s'%(first_name,last_name),userEmail)
