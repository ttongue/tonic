...
def buildMemberFormLine(plan):
   ...
   line="<input type=hidden name=memberplan value='"+plan+"'>"+plainTxt+" - 3 months included, then $"
...
def stepOneSelectPlan():
   outText=memberSystem.loadTemplate("templates/pdSignup-step1.html");
   tmpheader=header
...
def stepTwoGatherInfo(plan):
   template=memberSystem.loadTemplate("templates/pdSignup-step2.html")
...
def stepThreeConfirmation(plan,importantMessage):
   template=memberSystem.loadTemplate("templates/pdSignup-step3.html")
...
def stepFourProcess(plan):
   template=memberSystem.loadTemplate("templates/pdSignup-step4.html")
   results="<ul> "
   successmessage=" "
   inspectionResults=inspectFormInputsForMemberInfo()
...
# 
   ...
   print footer
   additionalComments=""
   adminEmail=memberSystem.loadTemplate("templates/pdStepFourEmail.txt")
   ...
   if (results.find("Error:") == -1):
       userEmail=memberSystem.loadTemplate("templates/pdSignupEmail.txt")
   ...
   if (step == ""):
	...
	stepOneSelectPlan()
      ...
      if ((sponsorRecord[memberSystem.DBCOL_MEMBER_NUMBER] != 1) |
         (memberSystem.isDayPassUnused(couponcode) == 0)):
	# It's not authorized for generating new members, or has been used
        errorText="Invalid OR Already Used Coupon Code!"
        stepOneSelectPlan()
        print "<!--"
        sys.exit(0)
